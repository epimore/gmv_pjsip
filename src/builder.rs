...
pub fn build_response(req: &SipMessage, opts: ResponseOptions) -> Bytes {
    ...
    if let Some(to) = req.header("To").or_else(|| req.header("t")) {
        out.push_str("To: ");
        if let Some(tag) = opts.to_tag.as_deref() {
            out.push_str(&ensure_tag(to, tag));
        } else {
            out.push_str(to);
        }
        out.push_str("\r\n");
    ...
    if let Some(contact) = opts.contact.as_deref() {
...
pub fn contact(platform_id: &str, host: &str, port: u16, proto: SipTransportProtocol) -> String {
    match proto {
        ...
        SipTransportProtocol::Tcp => {
            format!("<sip:{}@{}:{};transport=tcp>", platform_id, host, port)
        }
        SipTransportProtocol::Tls => {
            format!("<sips:{}@{}:{};transport=tls>", platform_id, host, port)
        }
    }
}
