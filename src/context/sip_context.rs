...
use crate::auth::{build_www_authenticate, verify_digest_response, AuthConfig, AuthDecision, NonceStore, VerifyDigestRequest};
use crate::builder::{build_request, build_response, new_branch, new_call_id, new_tag, via, ResponseOptions};
use crate::context::{
...
impl SipLocalConfig {
    pub fn local_uri(&self) -> String { format!("sip:{}@{}", self.platform_id, self.domain) }
    pub fn contact(&self, proto: SipTransportProtocol) -> String {
        crate::builder::contact(&self.platform_id, &self.public_host, self.listen_port, proto)
    }
}
...
impl SipContext {
    ...
    pub fn on_packet(&self, bytes: Bytes, meta: SipPacketMeta) -> Result<SipAction> {
        ...
        match &msg.kind {
            SipPacketKind::Request { method, .. } => self.on_request(msg.clone(), method.clone(), meta),
            SipPacketKind::Response { .. } => self.on_response(msg, meta),
        }
    }

    fn on_request(&self, msg: SipMessage, method: SipMethod, meta: SipPacketMeta) -> Result<SipAction> {
    ...
    fn on_response(&self, msg: SipMessage, _meta: SipPacketMeta) -> Result<SipAction> {
        ...
        if cseq.method.eq_ignore_ascii_case("INVITE") {
            if (100..200).contains(&code) {
                ...
                return Ok(SipAction::Event(SipEvent::InviteProceeding { call_id, status: code }));
            }
    ...
    fn handle_register(&self, msg: &SipMessage, meta: &SipPacketMeta) -> Result<SipAction> {
        ...
        match &auth {
            AuthDecision::Challenge { header_value, .. } => {
                let resp = build_response(msg, ResponseOptions {
                    status_code: 401,
                    server: Some(self.local.user_agent.clone()),
                    to_tag: Some(new_tag()),
        ...
        let authorized = match &auth { AuthDecision::Authorized { .. } | AuthDecision::Disabled => true, _ => false };
    ...
    fn handle_incoming_invite(&self, msg: &SipMessage, meta: &SipPacketMeta) -> Result<SipAction> {
        ...
        let trying = build_response(msg, ResponseOptions {
            status_code: 100,
            server: Some(self.local.user_agent.clone()),
        ...
        Ok(SipAction::SendManyAndEvent { bytes: vec![trying], event })
    }
    ...
    fn handle_ack(&self, msg: &SipMessage) -> Result<SipAction> {
        ...
        if let Some(dialog) = self.dialogs.get_by_call_id(&call_id) {
            self.dialogs.update_state(&dialog.id, DialogState::Confirmed);
        }
    ...
    fn handle_bye(&self, msg: &SipMessage) -> Result<SipAction> {
        ...
        self.calls.update_state(&call_id, InviteState::Terminated);
        if let Some(dialog) = self.dialogs.get_by_call_id(&call_id) {
            self.dialogs.update_state(&dialog.id, DialogState::Terminated);
        }
    ...
    fn authenticate_request(&self, msg: &SipMessage, method: &str) -> Result<AuthDecision> {
        if !self.local.auth.enabled { return Ok(AuthDecision::Disabled); }
        let Some(auth) = msg.header("Authorization") else {
            let nonce = self.nonces.issue();
            return Ok(AuthDecision::Challenge { header_value: build_www_authenticate(&self.local.auth.realm, &nonce, self.local.auth.algorithm), nonce });
        };
        let Some(provider) = self.local.auth.provider.as_ref() else { return Ok(AuthDecision::Forbidden("no password provider".into())); };
        let uri = msg.request_uri().unwrap_or_default();
        match verify_digest_response(VerifyDigestRequest { method, uri, authorization: auth, realm: &self.local.auth.realm, default_algorithm: self.local.auth.algorithm, nonce_store: &self.nonces, provider: provider.as_ref() }) {
            Ok(username) => Ok(AuthDecision::Authorized { username }),
            Err(e) => Ok(AuthDecision::Forbidden(e.to_string())),
        }
    }

    pub fn create_invite(&self, req: CreateInvite) -> Result<Bytes> {
        let call_id = req.call_id.unwrap_or_else(|| new_call_id(&self.local.public_host));
        ...
        let headers = vec![
            ("Via".into(), via(&self.local.public_host, self.local.listen_port, req.protocol, &branch)),
            ("Max-Forwards".into(), "70".into()),
            ("From".into(), format!("<{}>;tag={}", self.local.local_uri(), local_tag)),
        ...
        let bytes = build_request(&start_line, &headers, Some(Bytes::from(req.sdp.clone())), Some("application/sdp"));
    ...
    fn handle_invite_2xx_response(&self, msg: SipMessage) -> Result<SipAction> {
        let call_id = msg.call_id()?;
        let mut call = self.calls.get(&call_id).ok_or_else(|| SipError::CallNotFound(call_id.clone()))?;
        let remote_tag = msg.to_tag().ok_or_else(|| SipError::InvalidHeader { name: "To", reason: "2xx INVITE missing To tag".into() })?;
        let local_tag = msg.from_tag().ok_or_else(|| SipError::InvalidHeader { name: "From", reason: "2xx INVITE missing From tag".into() })?;
        let dialog_id = DialogId { call_id: call_id.clone(), local_tag, remote_tag };
        let remote_target = msg.contact().unwrap_or_else(|| format!("sip:{}@{}", call.device_id, self.local.domain));
    ...
    pub fn build_ack_for_invite_2xx(&self, call_id: &str) -> Result<Bytes> {
        let call = self.calls.get(call_id).ok_or_else(|| SipError::CallNotFound(call_id.into()))?;
        let dialog_id = call.dialog_id.clone().ok_or_else(|| SipError::DialogNotFound(call_id.into()))?;
        let dialog = self.dialogs.get(&dialog_id).ok_or_else(|| SipError::DialogNotFound(call_id.into()))?;
    ...
    pub fn create_bye(&self, req: CreateBye) -> Result<Bytes> {
        ...
        self.dialogs.update_state(&dialog_id, DialogState::Terminating);
        Ok(build_request(&start_line, &headers, None, None))
    }
    ...
    pub fn create_message(&self, req: CreateMessage) -> Result<Bytes> {
        ...
        let headers = vec![
            ("Via".into(), via(&self.local.public_host, self.local.listen_port, req.protocol, &branch)),
            ("Max-Forwards".into(), "70".into()),
            ("From".into(), format!("<{}>;tag={}", self.local.local_uri(), new_tag())),
            ("To".into(), format!("<{}>", req.target_uri)),
...
fn parse_expires(msg: &SipMessage) -> Option<u32> {
    msg.header("Expires").and_then(|v| v.trim().parse::<u32>().ok()).or_else(|| {
        msg.header("Contact").and_then(|contact| {
            contact.split(';').find_map(|p| {
                let (k, v) = p.trim().split_once('=')?;
                if k.eq_ignore_ascii_case("expires") { v.parse().ok() } else { None }
            })
        })
    })
}
...
fn auth_username(auth: &AuthDecision) -> Option<String> {
    match auth { AuthDecision::Authorized { username } => Some(username.clone()), _ => None }
}
