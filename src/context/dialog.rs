...
impl DialogStore {
    pub fn new() -> Self { Self { dialogs: DashMap::new(), by_call_id: DashMap::new() } }
    ...
    pub fn get(&self, id: &DialogId) -> Option<SipDialog> { self.dialogs.get(id).map(|v| v.clone()) }
    ...
    pub fn update_state(&self, id: &DialogId, state: DialogState) {
        if let Some(mut d) = self.dialogs.get_mut(id) { d.state = state; d.updated_at = Instant::now(); }
    }
...
impl Default for DialogStore { fn default() -> Self { Self::new() } }
